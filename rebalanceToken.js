...
async function main() {
  try {
    ...
    console.log(`Token balance: ${balance} DATA in wallet: ${walletAddress}`);

    if (balance > 20000) {
      const amountToSend = balance - 20000;
      if (amountToSend > 10) {
        console.log(`Sending ${amountToSend} DATA to ${destinationAddress}`);
        await send(privateKey, destinationAddress, amountToSend);
    ...
    console.error('Error retrieving token balance or sending funds:', error);
  }
}
